...
define('BASE_URL', '/archidep-git/comem-archidep-php-todo-exercise/');

// Database connection parameters.
define('DB_USER', 'todolist');
define('DB_PASS', 'Larry123!');
...
if (isset($_POST['action'])) {
  switch($_POST['action']) {
    ...
    case 'toggle':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $updateQuery = "UPDATE `todo` SET `done` = IF(`todo`.`done` = '0', '1', '0') WHERE `todo`.`id` = $id"; // IMPLEMENT ME | Done : Larry 08.10.2020 
    ...
    case 'delete':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $deleteQuery = "DELETE FROM `todo` WHERE `todo`.`id` =  $id"; // IMPLEMENT ME | Done : Larry 08.10.2020
 ...
 */
$selectQuery = 'SELECT * FROM `todo`'; // IMPLEMENT ME ¦ Done : Leyla 08.10.2020
